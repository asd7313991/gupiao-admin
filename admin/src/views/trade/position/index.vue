    ...
    <ElCard class="art-table-card"
      ...
      <ElTable v-loading="loading" :data="rows"
        ><ElTableColumn type="selection" width="42" /><ElTableColumn
          ...
          ></ElTableColumn
        ><ElTableColumn prop="currency" label="币种" /><ElTableColumn label="总市值"
          ><template #default="{ row }">{{ money(row.market_value) }}</template></ElTableColumn
    ...
    <ElDialog v-model="visible" :title="current?.id ? '编辑持仓' : '添加持仓'" width="680px"
      ><ElForm label-width="105px"
        ...
        ><ElFormItem label="可用数量"
          ><ElInputNumber v-model="form.available_qty" :min="0" :precision="0" /></ElFormItem
