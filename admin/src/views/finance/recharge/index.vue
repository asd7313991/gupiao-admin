  ...
  <div class="finance-page art-full-height"
    ...
    /><ElCard class="art-table-card"
      ><ArtTableHeader :loading="loading" @refresh="load"
        ><template #left
          ><ElButton @click="openEditor()">添加充值记录</ElButton></template
        ></ArtTableHeader
      ><div class="table-wrap"
        ><ElTable v-loading="loading" :data="rows" height="100%"
          ...
          /><ElTableColumn prop="customer_phone" label="手机号" /><ElTableColumn
            prop="customer_name"
            label="姓名"
          /><ElTableColumn label="充值凭证" width="100"
            ><template #default="{ row }"
              ><ElTag v-if="row.voucher" type="success">已上传</ElTag
              ><span v-else>-</span></template
            ></ElTableColumn
          ><ElTableColumn prop="method" label="充值方式" /><ElTableColumn label="充值金额"
            ><template #default="{ row }">{{ money(row.amount) }}</template></ElTableColumn
          ><ElTableColumn prop="currency" label="币种" /><ElTableColumn label="状态"
            ><template #default="{ row }"
          ...
          ><ElTableColumn prop="remark" label="备注" min-width="130" /><ElTableColumn
            prop="failure_reason"
            label="失败原因"
            min-width="130"
    ...
    ><ElDialog
      v-model="visible"
      :title="current?.id ? '编辑充值记录' : '添加充值记录'"
      width="560px"
      ><ElForm label-width="90px"
        ...
        ><ElFormItem label="充值方式"><ElInput v-model="form.method" /></ElFormItem
        ><ElFormItem label="状态"
...
<script setup lang="ts">
  ...
  import {
    ...
    saveFinanceRecharge,
    type FinanceRecharge
  } from '@/api/system-manage'
  defineOptions({ name: 'FinanceRecharge' })
  const loading = ref(false),
    rows = ref<FinanceRecharge[]>([]),
    total = ref(0),
    page = ref(1),
    pageSize = ref(20),
  ...
  const load = async () => {
    loading.value = true
    try {
      const data = await fetchFinanceRecharges({
        page: page.value,
  ...
  const visible = ref(false),
    current = ref<FinanceRecharge | null>(null),
    form = reactive({
      customer_id: 1,
      amount: 0,
      currency: 'CNY',
  ...
  const openEditor = (row?: FinanceRecharge) => {
  ...
  const save = async () => {
    if (!form.amount) return ElMessage.warning('请输入充值金额')
    await saveFinanceRecharge({ id: current.value?.id, ...form })
