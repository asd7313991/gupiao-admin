  ...
  <div class="setting-grid">
    <div v-for="field in fields" :key="field.key" class="setting-item">
      <div class="field-copy"
        ...
        ><small v-if="field.hint">{{ field.hint }}</small></div
      >
      <div class="field-control">
        ...
        <template v-else>
          <ElTag
            v-if="field.type === 'boolean'"
            :type="data[field.key] ? 'success' : 'info'"
            effect="light"
            >{{ data[field.key] ? '已开启' : '已关闭' }}</ElTag
          >
...
<script setup lang="ts">
  interface Field {
    key: string
    label: string
    type: 'number' | 'text' | 'time' | 'boolean'
    hint?: string
  }
  defineProps<{ editing: boolean; fields: readonly Field[] }>()
